...
tlStore 使用 `apps/tlstore-api` 作为 Nitro Backend for Frontend。`apps/tlstore-web` 不直接访问 PocketBase，不构建 PocketBase 地址，不读取或保存 PocketBase JWT。

## 请求边界

- 浏览器只调用同源 `/api/*`，Nginx 将其反向代理到内部 `tlstore-api` 容器。
...
- `NITRO_SESSION_SECURE=true` 时使用 `__Host-tlstore-session`、`Secure`、`HttpOnly`、`SameSite=Lax` Cookie。仅本地 HTTP 开发可设为 `false`，此时使用不带 `__Host-` 前缀的 Cookie。
