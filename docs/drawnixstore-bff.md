...
- `NITRO_WEB_ORIGIN` 为 HTTPS 时使用 `__Host-drawnixstore-session`、`Secure`、`HttpOnly`、`SameSite=Lax` Cookie；受控 HTTP 调试地址使用不带 `__Host-` 前缀的 HttpOnly Cookie。
- 当前镜像根据 `NITRO_WEB_ORIGIN` 的协议自动选择 Cookie 属性。`v0.2.0` 通过 `NITRO_SESSION_SECURE` 显式选择：HTTP 调试必须为 `false`，否则浏览器会拒绝 `Secure` Cookie，后续受保护接口将返回 `401`。变更该项后必须重建 API 容器、清除旧 Cookie 并重新登录。
- `NITRO_REGISTRATION_ENABLED` 默认 `false`。关闭时普通注册接口返回 `403`；只有系统尚无用户且 `NITRO_SETUP_TOKEN` 有效时，`/api/auth/setup` 才能创建首账号。初始化令牌使用恒定时间比较且仅在服务端环境读取。
- 初始化状态查询不返回令牌或用户数量。首账号与 PocketBase 内部集合的固定记录 ID 在同一批处理事务中创建，因此多 API 实例竞争或请求中断时不会产生第二个账号或残留申领；bootstrap 会为已有部署补齐该记录，避免升级后重新开放初始化。
